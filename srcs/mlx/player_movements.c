...
void	rotate_left_or_right(t_game *game, double angle)
{
	...
	game->player.dir_x = game->player.dir_x * cos(angle) - game->player.dir_y
		* sin(angle);
	game->player.dir_y = temp_dir_x * sin(angle) + game->player.dir_y
		* cos(angle);
	temp_cam_x = game->player.cam_x;
	game->player.cam_x = game->player.cam_x * cos(angle) - game->player.cam_y
		* sin(angle);
	game->player.cam_y = temp_cam_x * sin(angle) + game->player.cam_y
		* cos(angle);
}
...
{
	if (game->moove_backward || game->moove_forward || game->moove_left
		|| game->moove_right)
		moove_forward_backward_left_or_right(game, 0.05);
	if (game->rotate_right)
		rotate_left_or_right(game, 0.03);
